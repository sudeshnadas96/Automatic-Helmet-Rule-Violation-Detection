                        ...
                        if names[c] in LABELS:
                            ...
                            if names[c] == 'Two-wheeler':
                                ...
                                if head_count != helmet_count:
                                    ...
                                    filename = f'violations/helmet/{date}{Time}.jpg'
                                    cv2.imwrite(filename, im0[y-50:h, x:w])

                                    date1 = datetime.date.today().strftime('%Y-%m-%d')
                                    Time1 = datetime.datetime.now().time().strftime('%H:%M:%S')
                                    f = open("violations/records.csv", 'a', newline="")
                                    writer = csv.writer(f)
                                    writer.writerow(['no helmet', date1, Time1, f'{number}', filename])
                                    f.close()
                                ...
                                if head_count > 2:
                                    ...
                                    date = datetime.date.today().strftime('%Y%m%d')
                                    Time = datetime.datetime.now().time().strftime('%H%M%S')
                                    filename = f'violations/triple/{date}{Time}.jpg'
                                    cv2.imwrite(filename, im0[y-50:h, x:w])
...
def parse_opt():
    ...
    parser.add_argument('--source', type=str, default='test/cars.mp4', help='file/dir/URL/glob/screen/0(webcam)')
...
if __name__ == '__main__':
    ...
    if not os.path.exists("violations/records.csv"):
        f = open("violations/records.csv", 'w', newline="")
        writer = csv.writer(f)
        writer.writerow(['violation', 'Date', 'Time', 'numberplate', 'photo'])
        f.close()
